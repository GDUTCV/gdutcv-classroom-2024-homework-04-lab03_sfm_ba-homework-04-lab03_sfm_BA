...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    mtx = extrinsics[camera_idxs]
    mtx = np.einsum('ij,kjl->kil', intrinsics, mtx)

    homo_2_d = np.concatenate((points2d, np.ones((points2d.shape[0],1))), axis=1)
    homo_3_d = np.concatenate((points3d[points3d_idxs], np.ones((points3d[points3d_idxs].shape[0], 1))), axis=1)

    pro_2_d = np.einsum('kj,klj->kl', homo_3_d, mtx)
    pro_2_d = pro_2_d / np.repeat(pro_2_d[:,2].reshape(pro_2_d.shape[0], 1), 3, axis=1) # Normalizing

    residuals = np.linalg.norm(homo_2_d - pro_2_d, axis=1)
    """ END YOUR CODE HERE """
    return residuals
