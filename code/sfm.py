...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    """ YOUR CODE HERE """
    max_inliers = 0
    graph = {k : v.copy() for k, v in scene_graph.items()} # Used to prevent redundant checks

    for image_1 in scene_graph.keys():
        for image_2 in graph[image_1]:
            matches = load_matches(image_1, image_2)
            if len(matches) > max_inliers:
                max_inliers = len(matches)
                max_pair = [image_1, image_2]
            graph[image_2].remove(image_1)
    """ END YOUR CODE HERE """
    id1, id2 = sorted(max_pair)
    return id1, id2
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    """ YOUR CODE HERE """
    mtx = intrinsics @ np.concatenate((rotation_mtx, tvec), axis=1)
    
    for i in range(len(points3d)):
            reproj_2d = mtx @ np.append(points3d[i], 1).reshape(4, 1)
            norm_reproj_2d = reproj_2d / reprojected_2d[2]
            residuals[i] = np.linalg.norm(norm_reproj_2d - np.append(points2d[i], 1).reshape(3, 1))
    """ END YOUR CODE HERE """
    return residuals
